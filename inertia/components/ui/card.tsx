...
function CardHeader({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			...
			className={cn(
				'group/card-header @container/card-header grid auto-rows-min items-start gap-1 rounded-t-xl px-(--card-spacing) has-data-[slot=card-description]:grid-rows-[auto_auto] has-data-[slot=card-action]:@sm/card-header:grid-cols-[1fr_auto] [.border-b]:pb-(--card-spacing)',
				className,
			)}
			{...props}
		/>
...
function CardAction({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			...
			className={cn(
				'self-start justify-self-end @sm/card-header:col-start-2 @sm/card-header:row-span-2 @sm/card-header:row-start-1',
				className,
			)}
			{...props}
		/>
